package net.cdsunrise.ztyg.acquisition.protocol.rs485.component;
...
public class RS485Component {
    ...
    @Autowired
    private RS485ClientHandler rs485ClientHandler;
    @Autowired
    private VrvClientHandler vrvClientHandler;
    ...
    public String sendForVrv(String msg){
        return send2(msg,vrvIp,Integer.valueOf(vrvPort));
    }
    ...
    private RS485Msg send2(RS485Msg msg,String ip,Integer port){
        ChannelHandlerContext ctx = rs485ClientHandler.getCtx();
        if(ctx == null){
            connect(rs485BootStrap,ip,port);
            ctx = rs485ClientHandler.getCtx();
            if(ctx == null){
        ...
        // 发送数据
        ChannelFuture channelPromise = rs485ClientHandler.sendMessage(heapBuf);
        log.info(">>>>RS485Client请求设备数据:{}",msg.toString());
        try {
            channelPromise.await(2000L,TimeUnit.MILLISECONDS);
        } catch (Exception e) {}
        return rs485ClientHandler.getData();
    }

    @SuppressWarnings("Duplicates")
    private String send2(String msg,String ip,Integer port){
        ChannelHandlerContext ctx = vrvClientHandler.getCtx();
        if(ctx == null){
            connect(vrvBootStrap,ip,port);
            ctx =vrvClientHandler.getCtx();
            if(ctx == null){
                throw new BusinessException(ExceptionEnum.EXECUTE_ERROR);
            }
        }
        ByteBuf heapBuf = CodecUtils.getByteBuf(msg);
        // 发送数据
        ChannelFuture channelPromise = vrvClientHandler.sendMessage(heapBuf);
        log.info(">>>>VrvClient请求设备数据:{}",msg);
        try {
            channelPromise.await(2000L,TimeUnit.MILLISECONDS);
        } catch (Exception e) {}
        return vrvClientHandler.getData();
    }
